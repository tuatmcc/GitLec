...
ここでは, git のインストールと, GitHub のアカウント作成と, git の初期設定を行います. すでに済んでる人はスキップして OK です.
...
Git では, 変更されたファイルに 3 つの状態があります. 未追跡 (untracked), 変更あり (modified), ステージ済み (staged) です. 先ほど作成した `myname.txt` の状態を確認してみましょう. 以下のコマンドを実行します.
...
`-a` オプションを使用すると, 変更ありのファイルをすべてステージングしてからコミットします(新規作成したファイル,つまり未追跡ファイルはステージングされません). わざわざ `git add` コマンドを実行しなくてもよくなるので便利です. ただし, 新しく作成したファイルはステージングされないので注意してください. コミットしたら, `git log` コマンドを実行して, コミットログを確認してみましょう.
